...
const type = [
    [searchBarInput, 'click', focusHandler],
    [searchBarInput, 'keyup', resultHandler],
    [searchBarInput, 'keydown', eraseHandler],
    [searchResult, 'click', dropdownAnchorHandler],
];

type.forEach(([target, type, method])=>{
    target.addEventListener(type, method);
});

// 검색 결과 드롭다운 보이기
function focusHandler() {
    clearInput(false, 'block');
}

// ESC키로 검색창 초기화
function eraseHandler(ev){
    if(ev.key == 'Escape') {
        clearInput(true);
    }
}
...
function resultHandler(ev) {
    ...
    titles.forEach(function (title) {// object type의 titles
        let {name, bundleLine, fileName} = title;
...
function dropdownAnchorHandler(ev){
    let target = ev.target;
...
function clearInput(input, display='none'){
    if(input) searchBarInput.value = ''; // 검색어 삭제
    searchResult.style.display = display; // 드롭다운 닫기
}
// 검색창 end