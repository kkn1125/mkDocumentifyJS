...
- [Update List](#update-list)
  - [v1.0.0 버그 및 수정 사항](#v100-버그-및-수정-사항)
  - [v0.2.6 버그 수정 및 기능 추가](#v026-버그-수정-및-기능-추가)
  - [v0.2.5 버그 수정](#v025-버그-수정)
  - [v0.2.4 업데이트](#v024-업데이트)
    - [추가 사항](#추가-사항)
    - [버그 수정](#버그-수정)
...
1. 검색창
   - 검색어 입력 후 input 외 클릭 하고 다시 input 클릭 할 때 결과 리스트가 뜨지 않던 부분 수정
   - input style 변경
2. 채팅봇
   - 튜토리얼 기능 삭제

***kimson, ohoraming | 2021-11-11 12:09:21***

## v0.2.6 버그 수정 및 기능 추가

1. 채팅봇
   - 튜토리얼 기능 추가 (kimson tutorial v0.1.1)

***kimson | 2021-11-09 16:26:36***

## v0.2.5 버그 수정

1. snb(side bar) 반응형 지원
2. 채팅창
   - 반응형 지원
   - 사이즈 단위 수정
   - "도와줘" 명령어 딜레이 되지 않는 버그 수정
   - 메일 기능 연결 (kimson, ohoraming 문의 메일 확인완료)
3. GET /avatar... error 버그 발견 수정
   - 원인은 `img`의 `src`속성때문
   - 해결방안은 `img`태그 대신 `background-image`로 대체하여 이미지 삽입

***kimson | 2021-11-05 18:13:15***

## v0.2.4 업데이트

### 추가 사항

1. 검색창 기능 (MDN 검색 기능 참조)
2. 채팅창 버튼별 기능 연결
   - 업데이트, 문의, 만든사람, 저장 기능 연결
   - 페이지 저장 시 page object 전달
   - 메세지 입력 하단 스크롤 방식 변경
   - `userData.json`에 아바타 추가
   - bot, user 명칭 디스플레이 추가
   - 채팅창 사이즈 증가
3. 문서 전체 스크롤에서 메인 영역 스크롤 방식으로 변경
...
***kimson, ohoraming | 2021-11-03 19:09:02***
